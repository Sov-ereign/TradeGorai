...
export const TopNav: React.FC<TopNavProps> = ({
  ...
  const handleSaveCredentials = async () => {
    ...
    try {
      await saveZerodhaCredentials(apiKeyInput, apiSecretInput, accessTokenInput);
  ...
  return (
    <>
      <header className="h-14 bg-[#0D111A] border-b border-[#1E2638] px-4 flex items-center justify-between gap-4 sticky top-0 z-30">
        {/* Left: Global Search Stock */}
        <div className="relative w-80">
          <div className="relative flex items-center">
            ...
            <input
              ...
              placeholder="Search stock symbol (e.g. RELIANCE, TATAMOTORS)... [/]"
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[#121721] hover:bg-[#181F2C] border border-[#1E2638] text-xs text-slate-300 transition-colors"
          ...
          {/* Market Status Badge */}
          <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-950/40 border border-emerald-800/40 text-[11px] font-medium text-emerald-400">
            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
            <span>NSE: Open (09:15 - 15:30)</span>
          </div>

          {/* Zerodha Connection Status Button */}
          <button
            onClick={() => setShowConnectModal(true)}
            className={`flex items-center gap-2 px-3 py-1 rounded-lg text-xs font-semibold border transition-all ${
          ...
          >
            ...
            <span>
              {zerodhaStatus.connected ? 'Zerodha Live' : 'Kite Sandbox (Configure)'}
            </span>
          </button>
      ...
      {/* Zerodha Live Connection & OAuth Setup Modal */}
      {showConnectModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#121721] border border-[#1E2638] rounded-2xl w-full max-w-lg p-6 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            <button
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h2 className="text-base font-bold text-slate-100">Live Zerodha Kite Integration</h2>
                <p className="text-xs text-slate-400">Configure Kite API Credentials & Login via OAuth</p>
              </div>
            </div>
            ...
            <div className="space-y-3 mb-4">
              <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider flex items-center gap-1.5">
                ...
                Step 1: Enter Kite API Key & Secret (from kite.trade)
              </h3>
              ...
              <div>
                <label className="text-[11px] font-semibold text-slate-400 block mb-1">Access Token (Optional if logging in via OAuth)</label>
                <input
                  type="password"
                  placeholder="Paste 24-hour access token if generated manually..."
              ...
              {saveMessage && (
                <div className="p-2 rounded bg-[#182030] border border-emerald-500/30 text-xs text-emerald-300 font-medium">
                  {saveMessage}
                </div>
              )}
              ...
              <button
                ...
                className="w-full bg-[#182030] hover:bg-[#202B40] text-emerald-400 border border-emerald-500/40 font-semibold py-2 rounded-lg text-xs transition-colors"
              >
                Save Credentials
              </button>
            </div>
            ...
            <div className="pt-3 border-t border-[#1E2638] mb-4">
              ...
              <p className="text-[11px] text-slate-400 mb-3 leading-relaxed">
                Click below to open Zerodha's official login page. Once logged in, Zerodha automatically returns a session token back to TradeForge AI!
              </p>
              ...
              <a
                ...
                className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-2.5 px-4 rounded-xl text-xs flex items-center justify-center gap-2 transition-all shadow-lg shadow-emerald-950/40"
              >
